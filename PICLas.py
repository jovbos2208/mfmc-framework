...
def _expand_int_values(value, count: int, default: int) -> list[int]:
    if isinstance(value, (list, tuple, np.ndarray)):
        ...
        if arr.size >= count:
            return [int(v) for v in arr[:count]]
        pad = [int(arr[-1])] * (count - arr.size)
        return [int(v) for v in arr.tolist()] + pad
    try:
        scalar = int(value)
    except Exception:
        scalar = int(default)
...
class PiclasSimulator:
    ...
    def create_simulation_job_script(self, job_subdir):
        ...
        script_lines = [
            ...
            "  if echo \"$MPI_LIST\" | grep -Eq '(^|[^[:alnum:]_])pmix_v3([^[:alnum:]_]|$)'; then",
            "    SRUN_MPI_OPT=\"--mpi=pmix_v3\"",
            "  elif echo \"$MPI_LIST\" | grep -Eq '(^|[^[:alnum:]_])pmix([^[:alnum:]_]|$)'; then",
            "    SRUN_MPI_OPT=\"--mpi=pmix\"",
            "  elif echo \"$MPI_LIST\" | grep -Eq '(^|[^[:alnum:]_])pmi2([^[:alnum:]_]|$)'; then",
            "    SRUN_MPI_OPT=\"--mpi=pmi2\"",
            "  else",
    ...
    def collect_results(self, job_subdirs):
        ...
        Liefert für jedes Unterverzeichnis
            • globalen Gesamt-C_D  (Bezug A_ref = 0.5*wetted area, matching ADBSat)
            • CPU-Stunden
        -------------------------------------------------------------------------
        Rückgabe
        --------
        ...
        for subdir in job_subdirs:
            ...
            for fpath in result_files:
                ...
                #    \bar{C_D} =  Σ (f_i * A_i) / (q * A_ref)
                # --------------------------------------------------------------
                aw_cd = np.sum(cell_force * areas) / (dyn_p * A_ref)

                # --------------------------------------------------------------
                # 2) Gesamt-C_D   (Referenzfläche A_ref = 0.5*wetted area)
                #    Matches the current ADBSat coefficient convention.
                # --------------------------------------------------------------
    ...
    def submit_batch_postprocessing(self, batch_handles, random_seed, wait_for_completion=True):
        if isinstance(batch_handles, dict):
            handles = [batch_handles]
        else:
            handles = list(batch_handles)
        job_subdirs = []
